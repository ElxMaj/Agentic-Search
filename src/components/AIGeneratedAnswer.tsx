...
import { getRelatedContentForQuery } from '../data/relatedContentMockData';
...
const AIGeneratedAnswer: React.FC<AIGeneratedAnswerProps> = ({ 
  ...
  // Get related content based on the query
  const relatedContent = getRelatedContentForQuery(query);
  ...
  return (
    <>
      ...
      <RelatedContent items={relatedContent} isVisible={isVisible} />
    </>
  );
};
